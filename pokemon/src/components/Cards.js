...
export default function Card({ ...rest }) {
  ...
  if (pokedata == null) {
    ...
    axios
      .get(`https://pokeapi.co/api/v2/pokemon/${randomPokeId}`) // Getting the whole json data for a pokemon
      .then((res) => {
    ...
    >
      <Grid container alignItems="center" spacing={5} {...rest}>
        ...
        <Grid item sm={6} style={{ textAlign: 'left' }}>
          ...
          <Typography variant="h6" className={clsx(classes.card, activated)}>
